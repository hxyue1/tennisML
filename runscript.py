...
deuce_atp_cleaned = ta.data_cleaning(deuce_atp, 'Hard', ['Grand Slams', '250 or 500', 'Davis Cup', 'Masters', 'Challenger', 'Tour Finals'], 2000)
deuce_atp_long = ta.convert_long(deuce_atp_cleaned)
deuce_atp_long = ta.engineer_player_stats(deuce_atp_long)
...
ma_features, _ = ta.get_player_features(deuce_atp_long, tournaments, 2000, rolling_cols, last_cols, 60, 365)


wrangled_df = ta.wrangle_target(deuce_atp_cleaned, 2000, tournaments)
merged_data = ta.merge_data(wrangled_df, ma_features, 2000, tournaments, True)
df_final = ta.get_player_difference(merged_data)
...
train_indices, val_indices, test_indices = ta.f_chain_index_by_year(2000,2019, 20, True)
test_scores = []
val_scores = []
...
preds_df = ta.get_final_features(players, pd.to_datetime('2020-01-20'), deuce_atp_long, last_cols, rolling_cols, 60, 365)
